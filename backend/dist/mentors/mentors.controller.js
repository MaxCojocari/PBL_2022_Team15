"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MentorsController = void 0;
const common_1 = require("@nestjs/common");
const mentors_service_1 = require("./mentors.service");
let MentorsController = class MentorsController {
    constructor(mentorService) {
        this.mentorService = mentorService;
    }
    async addMentor(fN, lN, email, linkedin, twitter, ind, acc) {
        const mentorId = await this.mentorService.insertMentor(fN, lN, email, linkedin, twitter, ind, acc);
        return { id: mentorId };
    }
    async getAllMentors() {
        const mentors = await this.mentorService.getMentors();
        return mentors;
    }
    getOneMentor(mentorId) {
        return this.mentorService.getSingleMentor(mentorId);
    }
    async updateMentor(mentorId, fN, lN, email, linkedin, twitter, ind, acc) {
        await this.mentorService.modifyMentor(mentorId, fN, lN, email, linkedin, twitter, ind, acc);
        return null;
    }
    async removeMentor(mentorId) {
        const result = await this.mentorService.deleteMentor(mentorId);
        return result;
    }
};
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)('firstName')),
    __param(1, (0, common_1.Body)('lastName')),
    __param(2, (0, common_1.Body)('email')),
    __param(3, (0, common_1.Body)('linkedinURL')),
    __param(4, (0, common_1.Body)('twitterURL')),
    __param(5, (0, common_1.Body)('industries')),
    __param(6, (0, common_1.Body)('accelerators')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String, String, String, String, String]),
    __metadata("design:returntype", Promise)
], MentorsController.prototype, "addMentor", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], MentorsController.prototype, "getAllMentors", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], MentorsController.prototype, "getOneMentor", null);
__decorate([
    (0, common_1.Patch)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)('firstName')),
    __param(2, (0, common_1.Body)('lastName')),
    __param(3, (0, common_1.Body)('email')),
    __param(4, (0, common_1.Body)('linkedinURL')),
    __param(5, (0, common_1.Body)('twitterURL')),
    __param(6, (0, common_1.Body)('industries')),
    __param(7, (0, common_1.Body)('accelerators')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String, String, String, String, Array, Array]),
    __metadata("design:returntype", Promise)
], MentorsController.prototype, "updateMentor", null);
__decorate([
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], MentorsController.prototype, "removeMentor", null);
MentorsController = __decorate([
    (0, common_1.Controller)('mentors'),
    __metadata("design:paramtypes", [mentors_service_1.MentorsService])
], MentorsController);
exports.MentorsController = MentorsController;
//# sourceMappingURL=mentors.controller.js.map